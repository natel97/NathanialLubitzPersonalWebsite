...
def run_sudo(p_cmd, p_display=True):
  cmd = p_cmd
...
def get_service_status(p_svcname):
  ...
  if get_platform() == "Linux":
    if is_systemd():
      cmd = "systemctl status " + p_svcname
    else:
      cmd = "service " + p_svcname + " status"
    outp = commands.getoutput("sudo " + cmd)
    if ("stopped" in outp) or ("dead" in outp) or ("failed" in outp):
      return "Stopped"
    elif "running" in outp:
      return "Running"
...
def check_remote_host(p_reg, isJson=False):
  ...
  try:
    ...
    if host_info[1].startswith("~/"):
      user_info=remote.user_exists(host_info[2])
      pgc_home_dir=pgc_home_dir.replace("~",user_info.get('home'))
    pgc_home_exists = remote.is_exists(pgc_home_dir)
    if pgc_home_exists:
      if isJson:
        json_dict = {}
      ...
      if pgc_setup == 'yes':
        ...
        if not pgc_download_status:
            return False
        remote.upload_pgc("/tmp", os.path.split(pgc_home_dir)[0], pgc_version, repo)
      else:
        return False
    remote.disconnect()
  except Exception as e:
...
def run_pgc_ssh(p_svr, p_cmd,isJson=False, cache=False):
  ...
  is_stream=False
  stream_cmd = ("install", "update", "upgrade", "remove", "init")
  for c in stream_cmd:
      if p_cmd.find(c):
          is_stream=True
          break
  ...
  if is_stream:
    outlines = remote.execute_stream(pgc_cmd)
  else:
...
def get_os():
  ...
  try:
    ...
    if os.path.exists("/etc/lsb-release"):
      return(commands.getoutput("cat /etc/lsb-release | grep DISTRIB_CODENAME | cut -d= -f2"))
