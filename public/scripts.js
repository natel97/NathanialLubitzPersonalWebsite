...
$(function(){
	$('#skillWeb').click(function(){
		if (isrunning == false){
		$('#info').text("");
		information("I know how to work with html to organize the necessary tags in order to fufill the requirements of the website. I know how to insert images, videos, add click and drag functionality, and many other functions. I also know a lot of CSS and JavaScript. I've organized this particular page with a lot of CSS and some JavaScript functionality.", 0);
		}
		else{
		isrunning = false;
		}
	...
	$('#skillWindows').click(function(){
		if (isrunning == false){
			isrunning == true;
			$('#info').text("");
		information("I'm great at programming in C#. I know my way around LINQ, creating classes and instances of those classes, creating relavant fields for those objects, and putting it all to good use! I'm good at working with linked lists, too; however I prefer using databases when it gets to be like that! In class, I created a little banking application where you could create new accounts, deposit/withdrawl money, determine account time, gather required info(such as ID, address, etc.), and create bills to pay directly from the account. No real money was used during the process.", 0);
		}
		else{
			isrunning = false;
		}
...
$(function(){
	$('#skillApple').click(function(){
		if (isrunning == false){
			isrunning == true;
			$('#info').text("");
		information("In class, we worked with Objective-C and Swift. We installed Swift for Ubuntu and got to work. We also used JDoodle to interperet Objective-C", 0);
		}
		else{
			isrunning = false;
		}
...
$(function(){
	$('#skillGit').click(function(){
		if (isrunning == false){
			isrunning == true;
			$('#info').text("");
		information("I've done some research on GIT. We didn't learn this in class; however, it seems pretty easy to work with. I can initialize git, add all files to the location, push it, roll back changes, and clone from a repository. I can also add messages to comment what changes have been made. I also know how to obtain an SSH certificate and push to a remote server such as Github or Heroku.", 0);
		}
		else{
			isrunning = false;
		}
...
$(function(){
	$('#skillROR').click(function(){
		if (isrunning == false){
			isrunning == true;
			$('#info').text("");
		information("Ruby on Rails is one of the more fun things I've worked on. I created this website using Ruby On Rails! I really enjoyed it as it helped shine a little light on how blogs and dynamic websites are able to be run.", 0);
		}
		else{
			isrunning = false;
		}
